...
def send_mail(sender,app_password,receiver,subject,body,SendFile):
    msg=EmailMessage()
    ...
    with open(SendFile,'rb') as f:
        filedata=f.read()
        filepath=os.path.basename(SendFile)

        msg.add_attachment(
            filedata, maintype="application",subtype="octet-stream", filename=filepath
        )
...
def DataShield(Source="Data"):
    ...
    Zip_details=" created Zip file name is "+ zip_file

    send_mail(sender_mail,password,reciver_mail,subject,Zip_details,body)
